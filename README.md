...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_esm"></a> [esm](#module\_esm) | ./modules/esm | n/a |
| <a name="module_prometheus_exporter"></a> [prometheus\_exporter](#module\_prometheus\_exporter) | ./modules/prometheus_exporter | n/a |
...
| <a name="input_chart_namespace"></a> [chart\_namespace](#input\_chart\_namespace) | Namespace to install the chart into. | `string` | `"consul"` | no |
...
| <a name="input_chart_version"></a> [chart\_version](#input\_chart\_version) | Version of Chart to install. Set to empty to install the latest version | `string` | `"0.39.0"` | no |
| <a name="input_client_affinity"></a> [client\_affinity](#input\_client\_affinity) | affinity Settings for Client pods, formatted as a multi-line YAML string. | `string` | `null` | no |
...
| <a name="input_client_security_context"></a> [client\_security\_context](#input\_client\_security\_context) | Pod security context for client pods | `map(string)` | <pre>{<br>  "fsGroup": 1000,<br>  "runAsGroup": 1000,<br>  "runAsNonRoot": true,<br>  "runAsUser": 100<br>}</pre> | no |
...
| <a name="input_connect_inject_init_resources"></a> [connect\_inject\_init\_resources](#input\_connect\_inject\_init\_resources) | Resource settings for the Connect injected init container. | `map(any)` | <pre>{<br>  "limits": {<br>    "cpu": "50m",<br>    "memory": "50Mi"<br>  },<br>  "requests": {<br>    "cpu": "50m",<br>    "memory": "50Mi"<br>  }<br>}</pre> | no |
...
| <a name="input_connect_inject_resources"></a> [connect\_inject\_resources](#input\_connect\_inject\_resources) | Resources for connect inject pod | `map(any)` | <pre>{<br>  "limits": {<br>    "cpu": "50m",<br>    "memory": "50Mi"<br>  },<br>  "requests": {<br>    "cpu": "50m",<br>    "memory": "50Mi"<br>  }<br>}</pre> | no |
...
| <a name="input_consul_image_name"></a> [consul\_image\_name](#input\_consul\_image\_name) | Docker Image of Consul to run | `string` | `"hashicorp/consul"` | no |
| <a name="input_consul_image_tag"></a> [consul\_image\_tag](#input\_consul\_image\_tag) | Docker image tag of Consul to run | `string` | `"1.14.2"` | no |
...
| <a name="input_consul_k8s_tag"></a> [consul\_k8s\_tag](#input\_consul\_k8s\_tag) | Image tag of the consul-k8s binary to run | `string` | `"1.0.2"` | no |
...
| <a name="input_enable_consul_namespaces"></a> [enable\_consul\_namespaces](#input\_enable\_consul\_namespaces) | Make use of configuration beyond registering everything into the default Consul namespace. Consul ENT v1.7+ | `bool` | `false` | no |
...
| <a name="input_external_servers_hosts"></a> [external\_servers\_hosts](#input\_external\_servers\_hosts) | An array of external Consul server hosts | `list(string)` | `[]` | no |
| <a name="input_external_servers_https_port"></a> [external\_servers\_https\_port](#input\_external\_servers\_https\_port) | The HTTPS port of the Consul servers | `number` | `8501` | no |
| <a name="input_external_servers_k8s_authmethod_host"></a> [external\_servers\_k8s\_authmethod\_host](#input\_external\_servers\_k8s\_authmethod\_host) | If you are setting global.acls.manageSystemACLs and connectInject.enabled to true, set k8sAuthMethodHost to the address of the Kubernetes API server. This address must be reachable from the Consul servers. | `string` | `null` | no |
| <a name="input_external_servers_use_system_roots"></a> [external\_servers\_use\_system\_roots](#input\_external\_servers\_use\_system\_roots) | If true, consul-k8s-control-plane components will ignore the CA set in global.tls.caCert | `bool` | `false` | no |
...
| <a name="input_mesh_gateway_enabled"></a> [mesh\_gateway\_enabled](#input\_mesh\_gateway\_enabled) | If mesh gateways are enabled, a Deployment will be created that runs gateways and Consul Connect will be configured to use gateways. | `bool` | `false` | no |
...
| <a name="input_server_enabled"></a> [server\_enabled](#input\_server\_enabled) | If true, the chart will install all the resources necessary for a Consul server cluster. default is global.enabled | `string` | `"-"` | no |
...
| <a name="input_server_resources"></a> [server\_resources](#input\_server\_resources) | Resources for server | `any` | <pre>{<br>  "limits": {<br>    "cpu": "100m",<br>    "memory": "100Mi"<br>  },<br>  "requests": {<br>    "cpu": "100m",<br>    "memory": "100Mi"<br>  }<br>}</pre> | no |
| <a name="input_server_security_context"></a> [server\_security\_context](#input\_server\_security\_context) | Security context for server pods | `map(string)` | <pre>{<br>  "fsGroup": 1000,<br>  "runAsGroup": 1000,<br>  "runAsNonRoot": true,<br>  "runAsUser": 100<br>}</pre> | no |
...
| <a name="input_sync_resources"></a> [sync\_resources](#input\_sync\_resources) | Sync Catalog resources | `any` | <pre>{<br>  "limits": {<br>    "cpu": "50m",<br>    "memory": "50Mi"<br>  },<br>  "requests": {<br>    "cpu": "50m",<br>    "memory": "50Mi"<br>  }<br>}</pre> | no |
...
| <a name="input_tls_enabled"></a> [tls\_enabled](#input\_tls\_enabled) | Enable TLS for the cluster | `string` | `false` | no |
